...
export default function DiseaseDetection({ language }) {
  ...
  const handleImageUpload = async (file) => {
    ...
    reader.onload = async (e) => {
      ...
      await startScanning(base64Image, { cropType });
    };
  ...
  const startScanning = async (imageData, options = {}) => {
    ...
    try {
      ...
      const response = await fetch('http://localhost:8000/api/disease-detection/upload', {
        method: 'POST',
        body: formData,
      });
      ...
      // Format response to match DiseaseManagement structure
      const formattedResult = {
        disease: data.disease || 'Unknown Disease',
        confidence: data.confidence || 0,
        severity: data.confidence >= 85 ? 'high' : data.confidence >= 70 ? 'medium' : 'low',
        affected_area: 20,
        crop: cropType,
        model: 'ViT-Base Plant Disease Detection',
        summary: `Detected ${data.disease} with ${data.confidence}% confidence. Early intervention is recommended.`,
        treatment: {
          immediate: [
            'Isolate affected plants immediately',
            'Remove diseased leaves and destroy them',
        ...
        pesticides: [
          ...
          },
          {
            name: 'Sulfur Dust',
            type: 'Chemical',
            dosage: '4-5 kg per hectare',
            reEntryInterval: '48 hours'
          }
        ],
        organicAlternatives: [
          'Neem oil spray: 3-5% solution, spray every 7 days',
          'Bordeaux mixture: 1% solution for fungal diseases',
